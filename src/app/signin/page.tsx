  ...
  return (
    <div className="w-full max-w-xl bg-surface dark:bg-surface-dim rounded-modal shadow-sm border border-outline-variant/30 p-unit-lg md:p-unit-xl">
      <div className="text-center mb-unit-lg">
        ...
        <p className="font-body-md text-body-md text-on-surface-variant mt-unit-xs">Continue your spiritual journey.</p>
      </div>

      <form className="space-y-unit-md" onSubmit={handleSubmit}>
        {registered === "true" && (
          <div className="bg-tertiary-container/20 text-tertiary font-body-md text-body-md p-unit-sm rounded-input text-center">Account created successfully! Sign in below.</div>
        )}
        {error && (
        ...
        <div>
          <div className="flex items-center justify-between mb-unit-xs">
            ...
            <a href="/forgot-password" className="font-label-sm text-label-sm text-primary hover:text-primary-fixed-dim transition-colors">Forgot Password?</a>
          </div>
          <div className="relative">
            ...
            <input id="password" name="password" type="password" placeholder="Enter your password" required className="w-full h-12 pl-10 pr-10 bg-transparent border border-outline/40 rounded-input focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 font-body-md text-body-md text-on-surface placeholder:text-on-surface-variant/60 transition-all duration-200" />
          </div>
        </div>
      ...
      <div className="bg-primary-container/10 rounded-modal p-unit-md mt-unit-lg text-center">
        <p className="font-body-md text-body-md text-on-surface-variant">
          ...
          <a href="/signup" className="text-primary hover:text-primary-fixed-dim font-label-md text-label-md transition-colors">Sign Up</a>
        </p>
      </div>
    </div>
  );
...
export default function SignInPage() {
  return (
    <main className="flex-grow flex items-center justify-center bg-surface-container-low min-h-[calc(100vh-64px)] px-margin-mobile md:px-margin-desktop py-unit-xl">
      <Suspense fallback={<div className="w-full max-w-xl bg-surface dark:bg-surface-dim rounded-modal shadow-sm border border-outline-variant/30 p-unit-lg md:p-unit-xl text-center font-body-md text-body-md text-on-surface-variant">Loading...</div>}>
        <SignInForm />
      </Suspense>
    </main>
  );
