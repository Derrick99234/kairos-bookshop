...
const CATEGORY_ICONS: Record<string, string> = {
  ...
  "Breakthrough": "bolt", "Deliverance": "shield", "Healing": "healing",
  "Spiritual Warfare": "swords", "Prosperity": "account_balance", "Revival": "whatshot",
  "Finances/Giving": "payments", "Personal Development": "trending_up",
  "Pastoral ministry and Church Growth": "domain",
};
...
export default function Home() {
  ...
  function getBestVariant(book: Book): Variant | null {
    ...
    return book.variants.length > 0 ? book.variants[0] : null;
  }
    ...
    fetch("/api/categories")
      ...
      .then((cats) => {
        const sorted = [...(Array.isArray(cats) ? cats : [])].sort(
          (a, b) => (b._count?.books || 0) - (a._count?.books || 0)
        );
      ...
      <section className="relative bg-primary-container py-unit-xl md:py-32">
        ...
        <div className="relative max-w-4xl mx-auto px-margin-mobile text-center flex flex-col items-center gap-unit-lg">
          ...
          <div className="flex flex-wrap justify-center gap-unit-sm text-white/80 text-label-sm">
            ...
            <Link href="/books?search=Breakthrough" className="hover:text-white underline decoration-white/30">Breakthrough</Link>
            <Link href="/books?search=Prayer" className="hover:text-white underline decoration-white/30">Prayer</Link>
            <Link href="/books?search=Prophetic" className="hover:text-white underline decoration-white/30">Prophetic</Link>
            <Link href="/books?search=Faith" className="hover:text-white underline decoration-white/30">Faith</Link>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
          <div className="flex flex-col md:flex-row justify-between items-end mb-unit-xl gap-unit-md">
            <div>
              ...
              <p className="text-on-surface-variant font-body-md mt-unit-xs">The latest profound releases from Dr. Isaiah Macwealth.</p>
            </div>
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop">
          <div className="flex flex-col md:flex-row justify-between items-end mb-unit-xl gap-unit-md">
            <div>
              ...
              <div className="w-24 h-1 bg-secondary rounded-full mt-unit-sm" />
            </div>
            <Link className="text-secondary font-label-md flex items-center gap-unit-sm hover:gap-unit-md transition-all" href="/books">View All <span className="material-symbols-outlined">arrow_forward</span></Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-unit-md md:gap-gutter">
            {categories.slice(0, 6).map((cat) => (
              <Link key={cat.id} href={`/books?category=${cat.id}`} className="bg-surface-container-low p-unit-lg rounded-xl flex flex-col items-center justify-center gap-unit-sm text-center border border-outline-variant transition-all hover:shadow-md h-36 group">
                <span className="material-symbols-outlined text-[36px] group-hover:scale-110 transition-transform text-primary">{CATEGORY_ICONS[cat.name] || "category"}</span>
        ...
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop grid grid-cols-1 md:grid-cols-2 gap-unit-xl items-center">
          <div className="relative group">
            ...
            <div className="absolute -bottom-6 -right-6 bg-secondary text-white p-unit-md rounded-xl shadow-lg">
              <p className="font-headline-md text-xl">Dr. Isaiah Macwealth</p>
